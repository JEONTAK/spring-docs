package com.example.concurrent.domain.usercoupon.service;
...
import com.example.concurrent.domain.usercoupon.repository.UserCouponRepository;
...
public class UserCouponService {
    ...
    public void issueCouponUsingNamedLock(Long userId, Long couponId) {
        ...
        try (Connection conn = dataSource.getConnection()) {
            conn.createStatement().executeUpdate("SELECT GET_LOCK('" + lockName + "', 10)");
            ...
            try {
                ...
                conn.createStatement().executeUpdate("SELECT RELEASE_LOCK('" + lockName + "')");
            }
        } catch (SQLException e) {
            throw new CustomException(HttpStatus.INTERNAL_SERVER_ERROR, "NamedLock 처리 중 오류");
        }
    }

    public void issueCouponUsingLettuce(Long userId, Long couponId) {
        String lockKey = "coupon:lettuce:" + couponId;
    ...
    public void issueCouponUsingRedisson(Long userId, Long couponId) {
        RLock lock = redissonClient.getLock("coupon:redisson:" + couponId);
        try {
