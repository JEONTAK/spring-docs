...
public class UserController {
    ...
    @Order(1)
    @Operation(summary = "유저 삽입", description = "100만건의 유저를 랜덤으로 INSERT 합니다.")
    @PostMapping("/init")
    ...
    @Order(2)
    @Operation(summary = "기본 방식의 유저 조회 (이메일)", description = "해당 이메일 문자열이 포함되어 있는 유저들을 기본 방식으로 조회합니다.")
    ...
    @Order(3)
    @Operation(summary = "인덱싱을 사용한 유저 조회", description = "해당 이메일 문자열이 포함되어 있는 유저들을 인덱싱을 사용한 방식으로 조회합니다.")
    ...
    @Order(4)
    @Operation(summary = "기본 방식의 유저 조회 (이름)", description = "이름과 일치하는 유저들을 기본 방식으로 조회합니다.")
    ...
    @Order(5)
    @Operation(summary = "쿼리 최적화를 사용한 유저 조회 (필요한 컬럼만 조회)", description = "해당 이메일 문자열이 포함되어 있는 유저들을 쿼리 최적화를 사용한 방식으로 조회합니다.")
    ...
    @Order(6)
    @Operation(summary = "쿼리 최적화를 사용한 유저 조회 (페이징)", description = "페이징을 활용한 방식으로 유저를 조회합니다.")
    ...
    @Order(7)
    @Operation(summary = "배치 조회를 사용한 유저 조회 (IN)", description = "배치 조회(IN)을 활용한 방식으로 유저를 조회합니다.")
    ...
    @Order(8)
    @Operation(summary = "Redis Cache를 사용한 유저 조회", description = "Redis Cache를 활용한 방식으로 유저를 조회합니다.")
    ...
    @Order(9)
    @Operation(summary = "Caffeine Cache를 사용한 유저 조회", description = "Caffeine Cache를 활용한 방식으로 유저를 조회합니다.")
    ...
    @Order(10)
    @Operation(summary = "Async를 사용한 유저 조회", description = "Caffeine Cache를 활용한 방식으로 유저를 조회합니다.")
